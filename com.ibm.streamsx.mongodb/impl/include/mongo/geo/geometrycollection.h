...
namespace geo {
    ...
    public:
         ...
         */
        virtual GeoObjType getType() const { return GeoObjType_GeometryCollection; }
    ...
    template<typename TCoordinates>
    const std::vector<const GeoObj<TCoordinates>*>&
    GeometryCollection<TCoordinates>::getGeometries() const {
        return _geometries;
    }
    ...
    std::vector<const GeoObj<TCoordinates>*> GeometryCollection<TCoordinates>::parseGeometries(
        ...
        uassert(0, "bson must contain a field \"geometries\" of type Array",
