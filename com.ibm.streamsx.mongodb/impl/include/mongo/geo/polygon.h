...
namespace geo {
    ...
    public:
         ...
         */
        virtual GeoObjType getType() const { return GeoObjType_Polygon; }
    ...
    Polygon<TCoordinates>::Polygon(const Polygon<TCoordinates>& other)
        : _bson(other._bson)
        , _linearRings(other._linearRings) {
    ...
    BSONObj Polygon<TCoordinates>::createBSON(
        ...
        BSONObjBuilder bob;
        return bob.append(kTypeFieldName, kPolygonTypeStr)
    ...
    std::vector<LineString<TCoordinates> > Polygon<TCoordinates>::parseLinearRings(
        const BSONObj& bson) {
        std::vector<BSONElement> linearRingElems = Geometry<TCoordinates>::getCoordsField(
                bson).Array();
