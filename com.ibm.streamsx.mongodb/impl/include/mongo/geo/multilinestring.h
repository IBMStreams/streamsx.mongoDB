...
namespace geo {
    ...
    public:
         ...
         */
        virtual GeoObjType getType() const { return GeoObjType_MultiLineString; }
    ...
    std::vector<LineString<TCoordinates> > MultiLineString<TCoordinates>::parseLineStrings(
        const BSONObj& bson) {
        std::vector<BSONElement> lineStringArr = Geometry<TCoordinates>::getCoordsField(
                bson).Array();

        std::vector<LineString<TCoordinates> > lineStrings;
        for (size_t i = 0; i < lineStringArr.size(); ++i) {
            LineString<TCoordinates> line(Geometry<TCoordinates>::parsePointArray(
                    lineStringArr[i].Array()));
